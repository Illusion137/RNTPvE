...
export let Event = /*#__PURE__*/function (Event) {
   ...
   **/
  Event["SabrDownloadProgress"] = "sabr-download-progress";
  /**
   ...
   **/
  Event["SabrReloadPlayerResponse"] = "sabr-reload-player-response";
  /**
   * Fired when a PoToken refresh is needed.
   * Payload: { outputPath?: string, reason: 'placeholder_needed' | 'proactive' | 'expired' }
   * - `outputPath` is present for a background SABR download, and absent for active playback.
   * - 'placeholder_needed': fired when the server signals the placeholder token isn't sufficient (SPS=1)
   ...
   * Call `updateSabrPoToken` with the refreshed token for a download, or `updatePlaybackPoToken`
   * when `outputPath` is absent (active playback).
   **/
  Event["SabrRefreshPoToken"] = "sabr-refresh-po-token";
  return Event;
}({});
