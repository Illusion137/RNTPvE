...
export async function add(tracks, insertBeforeIndex = -1) {
  ...
  return addTracks.length < 1 ? undefined : TrackPlayer.add(addTracks.map(resolveTrackAssets), insertBeforeIndex);
}
 ...
 */
export async function updateTrackUrl(trackIndex, track) {
 ...
 */
export async function setCrossFade(seconds) {
  if (Platform.OS !== 'ios') {
    return;
  }
 ...
 */
export async function setEqualizer(bands) {
  if (Platform.OS !== 'ios') {
...
export async function getEqualizer() {
  ...
  return Array.isArray(result) && result.length > 0 ? result : [0, 0, 0, 0, 0];
}
 ...
 * (iOS only)
 */
export async function removeEqualizer() {
  if (Platform.OS !== 'ios') {
    // Equalizer is iOS only
    return;
  }
 ...
 */
export async function downloadSabr(params, outputPath) {
 ...
 */
export async function updateSabrStream(outputPath, serverUrl, ustreamerConfig) {
 ...
 */
export async function updateSabrPoToken(outputPath, poToken) {
 ...
 * @param poToken  The refreshed PoToken
 */
export async function updatePlaybackPoToken(poToken) {
